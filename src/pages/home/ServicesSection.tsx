          ...
          <div className="h-[253px] sm:h-[180px] lg:h-[253px] w-full rounded-md service-img transition-all delay-75 ease-out">
            {
              ...
              :
              <img src={defaultImg} alt={service?.title} className='h-full w-full object-cover rounded-md' />  
            }
          </div>
          <div className='service-title'>
            <h4 className='text-[25px] lg:text-[32px] leading-[30px] font-extrabold mt-4 lg:mt-6 mb-2'>{service?.title}</h4>
            <p className='text-[#566B84] service-details pb-8 pt-3'>{service?.description}</p>
          </div>

          <div className='space-y-4 pt-2'>
            {
            service?.facilities[0] !== '' && service?.facilities?.map((item:ReactNode) => <div className="flex items-center gap-x-2 lg:gap-x-[18px] tracking-wide">
